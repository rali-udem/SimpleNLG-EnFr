...
public class AdjectivePhraseTest extends SimpleNLG4TestBase {
	...
	public void testAdj() {
		...
		CoordinatedPhraseElement coordap = phraseFactory.createCoordinatedPhrase(
		...
		coordap.setFeature(Feature.CONJUNCTION, "but"); //$NON-NLS-1$
		Assert.assertEquals("incredibly salacious but amazingly beautiful", //$NON-NLS-1$
				this.realiser.realise(coordap).getRealisation());

		// changing the inner conjunction with WordElement
		WordElement or = (WordElement) phraseFactory.createWord("or", LexicalCategory.CONJUNCTION);
		...
		CoordinatedPhraseElement coord2 = phraseFactory.createCoordinatedPhrase(coordap,
				this.stunning);
		Assert.assertEquals(
		...
		// and stunning"
		CoordinatedPhraseElement preMod = phraseFactory.createCoordinatedPhrase(
